...
imgpath = "G:/123/漫画整理/21年11月/如履薄冰walk on water/第二部/0"

def main():
    imgdir = "G:/123/漫画整理/21年11月/如履薄冰walk on water/第二部/0" # 指定图片位置，形如：D:\Pictures
    ...
    for im in new_images:
        if im == new_images[0]:  # 第一张图片略过处理
            ...
            im = im.resize((new_images[0].size[0], int(new_images[0].size[0] * im.size[1] / im.size[0])),
                           Image.ANTIALIAS)
    ...
    imgcreate.save('final.png')  # 保存图片并以final.png命名，经测试，保存为png能尽可能地减少画质损失
